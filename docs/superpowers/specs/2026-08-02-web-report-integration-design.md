...
> **2026-08-02 개정.** 이 결정의 전제가 틀렸다. 아래 원문을 남기고 개정 내용을
> 이어 붙인다.

**원문.** `app/services/report_archive.py`는 PDF를 파일시스템에 쓴 뒤
...
**개정.** 위 두 이유는 지금도 옳다. 틀린 것은 **"기록을 남기려면 DB여야 한다"**
는 숨은 전제다.

실제로 확인된 동작은 이렇다. 렌더링·한글 글꼴 검사·파일 저장까지 전부 성공해
`var/reports/`에 읽을 수 있는 PDF가 12건 쌓였는데, 마지막 색인 행 한 줄을 넣지
못해 12건 모두 503으로 버려졌다. 사용자에게는 보고서가 **만들어지지 않은 것처럼**
보였다. 만들어졌고, 안전하게 저장돼 있었다.

그래서 `filesystem` 공급원을 이름 그대로 만든다 — 본문 옆에
`records/<xx>/<uuid>.json`으로 **기록도 파일시스템에 둔다.** 조회는 id만 들고
들어오므로 기록 경로는 `as_of`가 아니라 id 앞 두 자리로 나눈다.

이 개정은 원문의 두 이유를 지킨다.

- `GET /api/v1/reports/{id}.pdf`는 그대로 살아 있다. blob URL로 내려가지 않는다.
- 기준일·채택 절·정책 출처·본문 해시가 전부 기록에 남는다. 해시 검증도 그대로다.

DB 행은 이 저장소 안에서 아무도 읽지 않는 **부차적 색인**이 된다. 쓰지 못해도
문서는 조회되므로 보관을 실패로 만들지 않고 로그만 남긴다. 이 맞바꿈의 대가는
명시해 둔다: **DB를 운영 색인으로 쓰는 배포라면 조용히 색인만 빠질 수 있으므로
그 로그를 감시해야 한다.**
