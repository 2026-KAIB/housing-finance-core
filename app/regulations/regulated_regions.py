...
#           국토교통부 참고자료 「[참고] 투기과열지구 및 조정대상지역 추가 지정」
#           (주택정책과, 등록일 2026-06-30)
#           https://www.molit.go.kr/USR/NEWS/m_72/dtl.jsp?id=95092167
#           https://www.korea.kr/news/policyNewsView.do?newsId=148967354
#
# 전수 확인 기록 (`DESIGNATION_LIST_VERIFIED_THROUGH`의 근거)
#   [2026-07-30 확인] 국토교통부 참고·설명자료 게시판(m_72) 목록을 2026-06-29~
#           2026-07-29 구간까지 통째로 읽어 규제지역 고시가 더 없음을 확인했다.
#           https://www.molit.go.kr/USR/NEWS/m_72/lst.jsp
#           확인 시점 최신 게시물은 2026-07-29이며, 06-30 추가 지정 이후 투기과열
#           지구·조정대상지역 지정·해제 고시가 하나도 없다.
#           같은 구간의 2026-07-09 「토지거래허가구역 신규 지정」(호남 반도체
#           국가산단 예정지)은 **다른 제도**다. 토지거래허가구역은 `RegulationZone`
#           을 바꾸지 않으므로 이 표의 대상이 아니다 — 제도명이 비슷해 섞기 쉬워
#           일부러 남긴다.
#           규제지역 고시는 보도자료(m_71)가 아니라 참고·설명자료(m_72)에 오른다.
#           m_71만 보면 06-30 지정이 검색되지 않는다. 다음 확인 때도 m_72를 볼 것.
#
# 링크 주의: 과거 주석이 지정 현황 원표로 걸어 둔
#   https://www.molit.go.kr/policy/stable/sta_b_03.jsp
# 는 2026-07-30 현재 "페이지를 사용할 수 없습니다"를 반환한다(이전됨). 현황 원표
# 대신 위 고시 원문과 게시판 목록을 근거로 삼는다.
...
#
# 올릴 때는 **반드시 위 "전수 확인 기록"에 무엇을 어떻게 봤는지 함께 적는다.**
# 날짜만 올리면 이 장치가 막으려던 과대평가("목록에 없으니 비규제 → LTV 70%")가
# 근거 없이 열린다. 확인 없이 숫자만 미는 것이 이 파일에서 가장 위험한 변경이다.
#
# 2026-07-30 확인 시점의 게시판 최신 게시물은 2026-07-29였다. 즉 07-30 당일
# 게시물은 아직 없었고, 이 날짜는 "07-30 오전까지 새 고시 없음"을 뜻한다.
DESIGNATION_LIST_VERIFIED_THROUGH = date(2026, 7, 30)
